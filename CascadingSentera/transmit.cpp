...
void transmit::transmitRGBImage(uint8_t* uncompressedImage, int width, int height, int quality) {
	uint8_t* jpegBuf = nullptr;
	// uint8_t *uncompressedAr = compressor.getArrayFromMat(image);
	int jpegSize = compressor.compressRGBJpeg(uncompressedImage, &jpegBuf, width, height, quality);
	// indicate bgr with M
	loadBuffer(jpegBuf, jpegSize, 'M', true);
}

void transmit::transmitRGBPreCompressed(uint8_t *compressedImage, int size) {
	loadBuffer(compressedImage, size, 'M', false);
}

void transmit::transmitImage(uint8_t* uncompressedImage, int width, int height, int quality) {
	uint8_t* jpegBuf = nullptr;
...
void transmit::transmitImagePreCompressed(uint8_t* compressedImage, int size) {
...
void transmit::doTransmit(struct packet* transmissionPacket) {
	...
	uint8_t* transmission_data = new uint8_t[len]; // We may have over 256 packets ergo can't use 1 byte. We actually use 3 bytes for the packet index because it lets us
...
void transmit::loadBuffer(uint8_t* jpegBuf, int jpegSize, char type, bool isTjCompressed) {
